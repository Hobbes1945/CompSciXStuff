...
public class RandomWalk extends AbstractSimulation {
    ...
    public void doStep() {
        ...
        for (int i = 0; i < circle.length; i++) {
            circle[i].setY(circle[i].getY()+((int)(Math.random()*3))-1);
            circle[i].setX(circle[i].getX()+((int)(Math.random()*3))-1);
        }
    ...
    public void stop(){
        System.out.println(totalTime/10 + " secs to travel "
                + Math.abs(control.getDouble("Starting Y position") - circle[0].getY())+ " meters.");
    }
